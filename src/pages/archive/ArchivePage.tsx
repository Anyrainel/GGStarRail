...
function CatalogProvenance() {
  ...
  return (
    <section className="grid gap-3 lg:grid-cols-[minmax(0,1fr)_auto]">
            ...
            {manifest.schema_version === "1.1.0" && (
              <p className="mt-2 text-xs leading-5 text-muted-foreground">
                {t("archive.bundle.additiveSummary", {
                  ...
                  superimpositions: manifest.counts.light_cone_superimpositions,
      ...
      <div className="rounded-xl border border-border bg-card/65 p-4 lg:max-w-md">
        <div className="flex items-start gap-3">
          <ShieldAlert
            className="mt-0.5 h-5 w-5 shrink-0 text-primary"
            aria-hidden="true"
            ...
            <p className="mt-1 text-sm leading-6 text-muted-foreground">
              {diagnostics.data
                ? t("archive.gaps.summary", {
                    ...
                    disagreements: diagnostics.data.source_disagreements.length,
                  })
                : t("common.loading")}
            </p>
            <p className="mt-2 text-xs leading-5 text-muted-foreground">
